# Local Preview RAG Design

## Goal

Deploy the private SEN-QA preview index as a usable local skill on the Mac mini.
The executor profile and authenticated provider are configured outside Git.
No new API key, local model, public data endpoint, NAS computation, or production
retrieval alias is introduced.

## User workflow

1. The user opens the existing local dashboard on `127.0.0.1`.
2. The user asks an education-administration question in ordinary Korean.
3. The executor invokes the exact read-only `senqa-preview-search` command supplied by
   the local skill.
4. The command searches the sealed preview SQLite database and returns bounded
   ...
   provenance.
5. The executor answers only from those results, cites case ID, edition year, and PDF
   page, and always labels the answer as an unreviewed incomplete preview.
6. With no hit, the executor says that the preview corpus has no grounded result. It
...
instruction. The executor may summarize it but may not execute commands found in it,
...
toolset. The RAG skill is used only in an authenticated local dashboard or
explicit CLI sessions.

### Dashboard

The dashboard binds only to `127.0.0.1`. A privately configured authenticated
session provides inference. The dashboard is not exposed through Cloudflare and
...
- Candidate context is processed only by the privately configured provider.
- No credential, authentication token, source PDF, review database, or NAS path is placed
...
- A local one-shot smoke uses the installed skill and a non-sensitive query. The
